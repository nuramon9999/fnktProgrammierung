...
let calcScore (hand: string) =
    ...
    let sortedValues =
        if (straight) && flush.Contains(5) then
            ...
            230000 + (
                if (matchingCardVal.Item(0) > matchingCardVal.Item(1)) then
                    matchingCardVal.Item(0) * 100 + matchingCardVal.Item(1) * 10 + matchingCardVal.Item(2)
                else
                    matchingCardVal.Item(1) * 100 + matchingCardVal.Item(0) * 10 + matchingCardVal.Item(2)
            )
    ...
    printfn "%A" (sortedValues)
    sortedValues

let outp (hands:string list) =
    hands
    |> Seq.groupBy calcScore
    |> Seq.sortByDescending (fun (x,_) -> x)
    |> Seq.head |> snd |> Seq.toList

let toPrint = hands |> Seq.groupBy calcScore
printfn "%A" (Seq.toList toPrint)
